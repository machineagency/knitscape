...
function colorNumberGutterExtension(
  { state, parent },
  ...
  const view = () => {
    return html`<style>
      ...
      <div class="color-gutter ${gutterPos}">
        ${arr.map(
          (content, index) =>
...
export function colorNumberGutter(options = {}) {
  return (config) => colorNumberGutterExtension(config, options);
}
