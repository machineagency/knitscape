...
void main() {
    vec3 clip = (u_Matrix * vec3(a_Position.xy, 1));

    gl_Position = vec4(clip.xy, a_Position.z, 1);
}
`;
...
function initYarn(yarn) {
  ...
  const splinePts = new Float32Array(buildYarnCurve(yarn.pts));
  ...
  gl.vertexAttribPointer(
    ...
    gl.FLOAT, // type
    false, // normalize
    Float32Array.BYTES_PER_ELEMENT * 3, // stride
    0 // offset
  );
  ...
  return {
    ...
    instances: splinePts.length / 3 - 1,
  };
}
...
export function setup(yarnData, canvas) {
